...
function addNote(note) {
    ...
    // we specify that we have notes already, so we can add to it
    // we dont want our unique ids to get messed up later!
    let notes = currentNotes();

    // we pass our note into currentNotes() as argument
    notes.push(note);

    // we now add to our db.json file with new note
    
    /* here we are going to use the fs.writeFile() method, 
    this method writes data to the specified file and replaces it
    ...
    okay so fs.writeFile(file, JSON.stringify(notes, null, 2), callback) */
